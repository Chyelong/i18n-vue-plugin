...
本仓库可在 Codex 环境直接使用，核心能力基于 `skills/`、`agents/` 与脚本文件实现，不依赖 `.claude-plugin` 元数据。
...
- Codex 使用时不读取 `.claude-plugin/*`，直接按 skill 文档与脚本执行。
- 涉及“派发子代理”的步骤，Codex 中使用对应 agent 能力完成同等任务。
